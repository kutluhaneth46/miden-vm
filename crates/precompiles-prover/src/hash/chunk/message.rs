...
/// 2-tuple `(chunk_seq_id_head, absorption_id_head)` binding the head of
...
/// `bus_prefix[ChunkChain] + β⁰·chunk_seq_id_head + β¹·absorption_id_head`.
#[derive(Debug, Clone)]
pub struct ChunkChainMsg<E> {
    pub chunk_seq_id_head: E,
    pub absorption_id_head: E,
}
    ...
    EF: Algebra<E>,
{
    fn encode(&self, challenges: &Challenges<EF>) -> EF {
        challenges.encode(
            ...
            [self.chunk_seq_id_head.clone(), self.absorption_id_head.clone()],
        )
    }
}

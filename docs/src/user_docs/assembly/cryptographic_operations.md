...
| log_deferred <br /> - *(1 cycle)*   | [_, STMNT, _, ...] | [ROOT_NEW, STMNT, _, ...] | Folds `STMNT` from stack offsets 4..8 into the rolling deferred root with one raw Eidos compression: `ROOT_NEW = Eidos::compress(DEFERRED_AND_INIT_CV, ROOT_PREV \|\| STMNT)`. `STMNT` must be registered and evaluate to `TRUE`; only the top word is replaced. |
